...
hapFreqs <- fread("Reformatted_SEE01_hap_freqs_AllChems.txt", header = T)
uniquePos <- fread("Unique_chemPos_AllChems.txt", header = F)

chemPOS <- data.table(gpIdx = uniquePos)
chemPOS[, gp := gsub(".*-", "", gpIdx.V1)][, gp := gsub("\\..*", "", gp)]
...
job::job({
write.table(allCors, file=paste0(projectDir, "Data_Analysis/Sequencing_analysis/Tables/Correlation_tables/complete_chems_all_reps_spearman_cors.txt"), col.names = T, quote = F, sep = "\t")
